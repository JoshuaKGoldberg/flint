		...
		return {
			...
			files: {
					...
					"release.yaml": createSoloWorkflowFile({
						...
						steps: [
							...
							...builders.sort((a, b) => a.order - b.order).map(({ run }) => ({ run })),
							{
								...
								uses: resolveUses("JoshuaKGoldberg/release-it-action", "v0.3.2", options.workflowsVersions)
							}
						]
					})
				} },
				".release-it.json": JSON.stringify({
					...
					npm: { publishArgs: [`--access ${options.access}`, "--provenance"] },
...
//# sourceMappingURL=blockReleaseIt.js.map