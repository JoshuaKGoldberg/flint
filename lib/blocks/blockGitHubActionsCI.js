...
const blockGitHubActionsCI = base.createBlock({
	...
	intake({ files }) {
		...
		if (!setupNodeStep) return;
		const nodeVersion = setupNodeStep.with?.["node-version"];
		if (!nodeVersion) return;
		return { nodeVersion };
	},
	produce({ addons, options }) {
		const { jobs, nodeVersion = options.node.pinned ?? options.node.minimum } = addons;
		return {
			addons: [blockRepositoryBranchRuleset({ requiredStatusChecks: jobs?.map((job) => job.name) })],
			files: { ".github": {
				...
				workflows: {
					"ci.yaml": jobs && createMultiWorkflowFile({
	...
	transition() {
		return { addons: [blockRemoveFiles({ files: [".circleci", "travis.yaml"] })] };
	}
});

//#endregion
export { blockGitHubActionsCI };
//# sourceMappingURL=blockGitHubActionsCI.js.map