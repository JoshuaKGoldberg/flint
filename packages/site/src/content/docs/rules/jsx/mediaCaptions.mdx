...
Media elements (`<audio>` and `<video>`) must include a `<track>` element with `kind="captions"` to provide this accessibility feature. Muted media does not require captions.
