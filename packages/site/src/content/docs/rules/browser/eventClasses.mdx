...
<RuleSummary plugin="browser" rule="eventClasses" />

While `EventEmitter` is only available in Node.js, `EventTarget` is available in browsers, Deno, and modern Node.js (v14.5+).
Using `EventTarget` makes code more cross-platform friendly and can reduce bundle size in browser environments.
...
For new cross-platform code or browser-targeted code, prefer EventTarget.
...
<RuleEquivalents pluginId="browser" ruleId="eventClasses" />
