---
description: "Math, JSON, Reflect, and Atomics are not functions and should not be called directly."
title: "globalObjectCalls"
---

import { Tabs, TabItem } from "@astrojs/starlight/components";
import { RuleEquivalents } from "~/components/RuleEquivalents";
import RuleSummary from "~/components/RuleSummary.astro";

<RuleSummary plugin="ts" rule="globalObjectCalls" />

The global objects `Math`, `JSON`, `Reflect`, and `Atomics` are not constructors or functions.
They are built-in objects that provide utility methods and properties as namespaces.
Attempting to call them as functions or instantiate them with `new` will result in a TypeError at runtime.

## Examples

<Tabs>
<TabItem label="❌ Incorrect">

```ts
const result = Math();

const data = JSON();

const reflected = Reflect();

const instance = new Atomics();
```

</TabItem>
<TabItem label="✅ Correct">

```ts
const value = Math.abs(-5);

const parsed = JSON.parse("{}");

const keys = Reflect.ownKeys({});

const result = Atomics.add(buffer, 0, 1);
```

</TabItem>
</Tabs>

## Options

This rule is not configurable.

## When Not To Use It

This rule should generally always be enabled, as calling these global objects as functions will always result in a runtime error.
You might only disable it if you're intentionally redefining these names in your code (though this is not recommended).

## Further Reading

- [MDN: Math](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math)
- [MDN: JSON](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON)
- [MDN: Reflect](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Reflect)
- [MDN: Atomics](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Atomics)

## Equivalents in Other Linters

<RuleEquivalents pluginId="ts" ruleId="globalObjectCalls" />
