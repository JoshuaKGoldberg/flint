...
```ts
// Or even better, just use the async function directly:
const result = async () => {
...
If you have a specific use case where you intentionally need to use an async function as a Promise executor, you can disable this rule.
However, this is generally considered an anti-pattern and should be avoided.
