...
excerpt: Flint is an experimental linter.
  It intentionally revisits many of the core legacy design decisions other popular web linters have gone by for years.
...
This follows the same broad path as [Oxlint type-aware linting](https://voidzero.dev/posts/announcing-oxlint-type-aware-linting).
...
Flint does not mark a distinction between type-aware rules vs. untyped rules.
...
We believe there is a strong correlation between users who want to lint and those whose projects are worth the overhead of TypeScript.
...
Astro, Svelte, Vue, and other extension languages add to TypeScript syntax and even completely replace it at times.
However, no existing mainstream provides full API hooks for plugins to add their extension languages into TypeScript.
...
Flint's Vue plugin, for example, will be able to support fully typed `*.vue` imports by integrating [Volar.js](https://volarjs.dev) with Flint's TypeScript extensibility APIs.
...
- Native speed linters such as Biome and Oxlint run so quickly (prior to typed linting) that a cache is not generally worthwhile
...
Users are of course still be able to write their own plugins for other languages.
...
The "one file at a time" model all of today's linters doesn't lend itself well to all the operations a codemod might need.
...
All lint rule reports are treated as lint errors and will be visualized with the same non-red color -e.g. yellow or orange- squigglies in errors.
...
Third-party plugins are be encouraged -but not required- to use those categories as well.

That split between `logical` and `stylistic` allows developers to react to the metadata in their own tooling.
For example, developers might want to use an equivalent of `eslint.rules.customizations` to downgrade stylistic rules to blue info squigglies instead of yellow :
...
- Singularity: the least troublesome, but still a general point of confusion
...
Here are some canonical names from Flint's TypeScript (and JavaScript) plugin:
...
Flint's rules attempt make the same choice consistently for their options:
...
Here are some canonical options from Flint's TypeScript (and JavaScript) plugin:
...
```json
{
	"my-plugin/some-rule": ["error", { "setting": "..." }]
}
```
...
They will allow [Standard Schema](https://standardschema.dev) descriptions for rule options, which allows TypeScript-friendly schema validations libraries such as [Zod](https://zod.dev):

```ts
import { typescriptLanguage } from "@flint/ts";
import { z } from "zod";
...
As a result, Flint's configuration is _fully typed_.
...
In practice, there are approximately no plugins in common use that share settings with each other.

Flint will allow plugins to define their own settings with Standard Schema types.
...
You can find the Flint glossary on [flint.fyi/glossary](https://flint.fyi/glossary).
...
Flint's "incubator" plugins are each intended to eventually grow into those community projects.
...
Flint will need to provide a compatibility layer to include and those plugins and rules natively via a Flint configuration file.

[ESLint's compatibility utilities](https://eslint.org/blog/2024/05/eslint-compatibility-utilities) and [`eslint-plugin-tslint`](https://github.com/typescript-eslint/eslint-plugin-tslint) good references of prior art for compatibility layers.
...
Those flags would allow the migration tool to be used as more than just a single-shot utility.
